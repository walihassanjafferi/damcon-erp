...
@section('content')
    ...
    <style>
        ...
        #supplier_pos_table thead tr th:nth-child(6) {
            display: none;
        }
        #supplier_pos_table tbody tr td:nth-child(6) {
            display: none;
        }
    </style>
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">Supplier Purchase Orders Management</h4>

            <a href="{{ route('supplierspos.create') }}" class="create-btn"><i data-feather='plus'></i> Add Supplier Order</a>
            <p class="card-text">
            </p>
            <table class="table table-striped table-bordered supplier_pos_table" style="width:100%" id="supplier_pos_table">
                <thead>
                <tr>
                    <th>#</th>
                    <th>ID</th>
...
@section('scripts')
    <script>
        $(document).ready( function () {
            ...
            var table = $('.supplier_pos_table').DataTable({
                ...
                buttons: [
                    {
                        ...
                        filename:"Supplier Purchase Orders Data",
                    },
                    {
                        extend: 'pdfHtml5',
                        title: 'Supplier Purchase Orders',
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('supplierpos_get_orders') }}",
                columns: [
                    {data: 'checkbox', name: '#'},
            ...
            $('#supplier_pos_table tbody').on( 'click', 'tr', function () {
        ...
        function deleteOrder(id){
            ...
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this Supplier Purchase Order",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                    ...
                    if (willDelete) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $('#supplier_pos_table').DataTable().row('.deleted').remove().draw( false );
                            },
                            error: function(data)
                            {
                                alert('Error Failed');
            ...
            if(List_ID.length>0){
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            $.ajax({
                                url:'{{route('supplierpos_bulk_remove')}}',
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "order_list":List_ID
                                },
