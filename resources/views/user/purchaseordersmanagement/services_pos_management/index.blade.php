...
@section('content')
    ...
    <style>
        ...
        #services_pos_table thead tr th:nth-child(7) {
            display: none;
        }
        #services_pos_table tbody tr td:nth-child(7) {
            display: none;
        }
        #services_pos_table thead tr th:nth-child(9) {
            display: none;
        }
        #services_pos_table tbody tr td:nth-child(9) {
            display: none;
        }
    </style>
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">Maintenance Items & Services POs Management</h4>

            <a href="{{ route('servicespos.create') }}" class="create-btn"><i data-feather='plus'></i> Add Purchase Order</a>

            <p class="card-text">
            </p>

            <table class="table table-striped table-bordered services_pos_table" style="width:100%" id="services_pos_table">
                <thead>
                <tr>
                    <th>#</th>
                    <th>ID</th>
                    <th>Title PO Number </th>
                    <th>Supplier Name</th>
                    <th>Tax Body</th>
                    <th>Issue Date</th>
                    <th>Issue Date</th>
...
@section('scripts')
    <script>
        $(document).ready( function () {
            ...
            var table = $('.services_pos_table').DataTable({
                ...
                buttons: [
                    {
                        ...
                        filename:"Services Purchase Orders Data",
                    },
                    {
                        extend: 'pdfHtml5',
                        title: 'Services',
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('servicespos_get_orders') }}",
                columns: [
                    {data: 'checkbox', name: '#'},
                    {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                    {data: 'title_po_number', name: 'title_po_number'},
                    {data: 'supplier_name', name: 'supplier_name'},
                    {data: 'taxbody', name: 'taxbody'},
            ...
            $('#services_pos_table tbody').on( 'click', 'tr', function () {
        ...
        function deleteOrder(id){
            ...
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this Services Purchase Order",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                    ...
                    if (willDelete) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $('#services_pos_table').DataTable().row('.deleted').remove().draw( false );
                            },
                            error: function(data)
                            {
                                alert('Error Failed');
            ...
            if(List_ID.length>0){
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            $.ajax({
                                url:'{{route('servicespos_bulk_remove')}}',
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "order_list":List_ID
                                },
