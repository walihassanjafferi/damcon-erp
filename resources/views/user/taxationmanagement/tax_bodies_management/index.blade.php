...
@section('title')
    <title>Damcon ERP -  Tax Bodies Management List</title>
@endsection
@section('content')
    @include('alert.alert')
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">Tax Bodies Management</h4>

            <a href="{{ route('tax_bodies.create') }}" class="create-btn"><i data-feather='plus'></i> Add </a>

            <p class="card-text">
            </p>

            <table class="table table-striped table-bordered tax_bodies_tabel" style="width:100%" id="tax_bodies_tabel">
                <thead>
                <tr>
                    <th>#</th>
                    <th>ID</th>
...
@section('scripts')
    <script>
        $(document).ready( function () {
            ...
            var table = $('.tax_bodies_tabel').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Tax Bodies Management',
                        className: 'btn btn-primary',
                        exportOptions: {
                            columns: [  1, 2, 3, 4,5]
                        },
                        filename:"Tax Bodies Management Data",
                    },
                    {
                        extend: 'pdfHtml5',
                        title: 'Tax Bodies Management',
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('tax_bodies.index') }}",
                columns: [
                    {data: 'checkbox', name: '#'},
                    {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                    {data: 'name', name: 'name'},
                    {data: 'sales_tax_percentage_items', name: 'sales_tax_percentage_items'},
                    {data: 'sales_tax_percentage_services', name: 'sales_tax_percentage_services'},
                    {data: 'rule_creation_date', name: 'rule_creation_date'},
                    {
                        data: 'action',
                        name: 'action',
                        orderable: true,
            ...
            $('#tax_bodies_tabel tbody').on( 'click', 'tr', function () {
                    ...
                    if (willDelete) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $('#tax_bodies_tabel').DataTable().row('.deleted').remove().draw( false );
                                $("#th_check_box").prop('checked',false);
                            },
                            error: function(data)
                            {
            ...
            if(List_ID.length>0){
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            $.ajax({
                                url:'{{route('tax_bodies_bulk_remove')}}',
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "order_list":List_ID
                                },
