...
@section('content')
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">Supplier Tax Payments Management</h4>

            <a href="{{ route('suppliertaxpayment.create') }}" class="create-btn"><i data-feather='plus'></i> Add </a>

            <p class="card-text">
            </p>

            <table class="table table-striped table-bordered supplier_tax_management_tabel" style="width:100%" id="supplier_tax_management_tabel">
                <thead>
                <tr>
                    <th>#</th>
                    <th>Title </th>
                    <th>Supplier Name</th>
                    <th>Payable Tax (PKR)</th>
...
@section('scripts')
    <script>
        $(document).ready( function () {
            ...
            var table = $('.supplier_tax_management_tabel').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Supplier Tax Payments Management',
                        className: 'btn btn-primary',
                        exportOptions: {
                            columns: [  1, 2, 3, 4,5]
                        },
                        filename:"Supplier Tax Payments Management Data",
                    },
                    {
                        extend: 'pdfHtml5',
                        title: 'Supplier Tax Payments Management',
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('suppliertaxpayment.index') }}",
                columns: [
                    {data: 'checkbox', name: '#'},
                    {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                    {data: 'title', name: 'title'},
                    {data: 'payable_tax', name: 'payable_tax'},
            ...
            $('#supplier_tax_management_tabel tbody').on( 'click', 'tr', function () {
        ...
        function deleteOrder(id){
            ...
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this Supplier Tax",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                .then((willDelete) => {
                    ...
                    if (willDelete) {
                        $.ajax({
                            url:'{{route('suppliertaxpayment.destroy','id')}}',
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "id":id
                            },
                            ...
                            success: function(data) {
                                ...
                                $('#supplier_tax_management_tabel').DataTable().row('.deleted').remove().draw( false );
                                $("#th_check_box").prop('checked',false);
                            },
                            error: function(data)
                            {
