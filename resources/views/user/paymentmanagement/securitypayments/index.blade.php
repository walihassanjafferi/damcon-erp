...
@section('title')
    <title>Damcon ERP -  Security Payments Management List</title>
@endsection
@section('content')
    @include('alert.alert')
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">Security / Bid Bond Payments Management</h4>

            <a href="{{ route('security_payment_management.create') }}" class="create-btn"><i data-feather='plus'></i> Add Security/Bid Bond Payments</a>

            <p class="card-text">
            </p>

            <table class="table table-striped table-bordered security_payments_table" style="width:100%" id="security_payments_table">
                <thead>
                <tr>
                    {{-- <th>#</th> --}}
                    <th>#</th>
...
@section('scripts')
    <script>
        $(document).ready( function () {
            ...
            var table = $('.security_payments_table').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Security Payments Management',
                        className: 'btn btn-primary',
                        exportOptions: {
                            columns: [  1, 2, 3, 4,5,6]
                        },
                        filename:"Security Payments Management Data",
                    },
                    {
                        extend: 'pdfHtml5',
                        title: 'Security Payments Management',
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('security_payment_management.index') }}",
                columns: [
                    // {data: 'checkbox', name: '#'},
            ...
            $('#security_payments_table tbody').on( 'click', 'tr', function () {
        ...
        function deleteOrder(id){
            ...
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this Security Payment",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                    ...
                    if (willDelete) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $('#security_payments_table').DataTable().row('.deleted').remove().draw( false );
                                $("#th_check_box").prop('checked',false);
                            },
                            error: function(data)
                            {
        ...
        function DeleteOrders(){
            ...
            if(List_ID.length>0){
                swal({
                    title: "Are you sure?",
                    text: "Once deleted, you will not be able to recover this Security Payments",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
                    .then((willDelete) => {
                        if (willDelete) {
                            $.ajax({
                                url:'{{route('security_payment_bulk_remove')}}',
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "order_list":List_ID
                                },
                                ...
                                success: function(data) {
                                    ...
                                    if($(".deleted").length >= 10){
                                        $('#security_payments_table').DataTable().ajax.reload(null, false);
                                    }else{
