...
@section('title')
    <title>Damcon ERP -  Supplier Payments Management List</title>
@endsection
@section('content')
    @include('alert.alert')
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">Supplier Payments Management</h4>

            <a href="{{ route('supplier_payment_management.create') }}" class="create-btn"><i data-feather='plus'></i> Add </a>

            <p class="card-text">
            </p>

            <table class="table table-striped table-bordered supplier_payments_table" style="width:100%" id="supplier_payments_table">
                <thead>
                <tr>
                    <th>#</th>
                    <th>ID</th>
    ...
    <script>
        $(document).ready( function () {
            ...
            var table = $('.supplier_payments_table').DataTable({
                ...
                buttons: [
                    {
                        ...
                        filename:"Supplier Payments Management Data",
                    },
                    {
                        extend: 'pdfHtml5',
                        title: 'Supplier Payments Management',
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('supplier_payment_management.index') }}",
                columns: [
                    {data: 'checkbox', name: '#'},
            ...
            $('#supplier_payments_table tbody').on( 'click', 'tr', function () {
        ...
        function deleteOrder(id){
            ...
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this Supplier Payment",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                    ...
                    if (willDelete) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $('#supplier_payments_table').DataTable().row('.deleted').remove().draw( false );
                                $("#th_check_box").prop('checked',false);
                            },
                            error: function(data)
                            {
        ...
        function DeleteOrders(){
            ...
            if(List_ID.length>0){
                swal({
                    title: "Are you sure?",
                    text: "Once deleted, you will not be able to recover this Supplier Payments",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
                    .then((willDelete) => {
                        if (willDelete) {
                            $.ajax({
                                url:'{{route('supplier_payment_bulk_remove')}}',
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "order_list":List_ID
                                },
