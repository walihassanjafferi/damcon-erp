...
@section('title')
    <title>Damcon ERP -  Bank Payments Management List</title>
@endsection
@section('content')
    @include('alert.alert')
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">Bank Payments Management</h4>

            <a href="{{ route('bank_payment_management.create') }}" class="create-btn"><i data-feather='plus'></i> Add </a>
...
@section('scripts')
    <script>
        $(document).ready( function () {
            ...
            var table = $('.batches_table').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Bank Payments Management',
                        className: 'btn btn-primary',
                        exportOptions: {
                            columns: [  1, 2, 3, 4,5]
                        },
                        filename:"Bank Payments Management Data",
                    },
                    {
                        extend: 'pdfHtml5',
                        title: 'Bank Payments Management',
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('bank_payment_management.index') }}",
                columns: [
                    {data: 'checkbox', name: '#'},
        ...
        function deleteOrder(id){
            ...
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this Bank Payment",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
            ...
            if(List_ID.length>0){
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            $.ajax({
                                url:'{{route('bank_payment_bulk_remove')}}',
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "order_list":List_ID
                                },
