...
@section('content')
    ...
    <div class="card mb-4">
        <div class="card-body">
            ...
            <a href="{{ route('leave-management.create') }}" class="create-btn"><i data-feather='plus'></i> Add Leave</a>

            <p class="card-text">
            <table class="table table-striped table-bordered leave_table" style="width:100%" id="leave_table">
                <thead>
                <tr>
                    <th></th>
                    <th>#</th>
                    <th>Name</th>
                    <th>Cnic</th>
                    <th>Damcon Employee ID</th>
                    <th>Pre-Annual leaves</th>
                    <th>Pre-Casual leaves</th>
                    <th>Pre-Sick leaves</th>
                    <th>Pre-Off leaves</th>
    ...
    <script>
        $(document).ready( function () {
            ...
            var table = $('.leave_table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('getAjaxleaves') }}",
                columns: [
                    {data: 'checkbox', name: '#'},
                    {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                    {data: 'name', name: 'name'},
                    {data: 'cnic', name: 'cnic'},
                    {data: 'damcon_emp_id', name: 'damcon_emp_id'},
                    {data: 'annual_leaves', name: 'annual_leaves'},
                    {data: 'casual_leaves', name: 'casual_leaves'},
                    {data: 'sick_leaves', name: 'sick_leaves'},
                    {data: 'off_leaves', name: 'off_leaves'},
                    {
                        data: 'action',
                        name: 'action',
                        orderable: true,
            ...
            $('#leave_table tbody').on( 'click', 'tr', function () {
