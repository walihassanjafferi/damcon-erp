...
@section('title')
    <title>Damcon ERP -  View Damcon Assets Depreciation</title>
@endsection
@section('content')
    @include('alert.alert')
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">Damcon Assets Depreciation</h4>

            <a href="{{ route('damconassetsdepreciation.create') }}" class="create-btn"><i data-feather='plus'></i> Add Assets</a>

            <p class="card-text">
            </p>

            <table class="table table-striped table-bordered damcon_assets_table" style="width:100%" id="damcon_assets_table">
                <thead>
                <tr>
                    <th>#</th>
                    <th>ID</th>
...
@section('scripts')
    <script>
        $(document).ready( function () {
            ...
            var table = $('.damcon_assets_table').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Damcon Assets Depreciation',
                        className: 'btn btn-primary',
                        exportOptions: {
                            columns: [  1, 2, 3, 4,5,6]
                        },
                        filename:"Damcon Assets Depreciation Data",
                    },
                    {
                        extend: 'pdfHtml5',
                        title: 'Damcon Assets Depreciation',
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('assets.depreciation_get') }}",
                columns: [
                    {data: 'checkbox', name: '#'},
                    ...
                    {data: 'asset_new_price', name: 'asset_new_price'},
            ...
            $('#damcon_assets_table tbody').on( 'click', 'tr', function () {
        ...
        function deleteOrder(id){
            ...
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this Damcon Assets Depreciation",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                    ...
                    if (willDelete) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $('#damcon_assets_table').DataTable().row('.deleted').remove().draw( false );
                                $('#th_check_box').prop('checked',false);
                            },
                            error: function(data)
                            {
            ...
            if(List_ID.length>0){
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            $.ajax({
                                url:'{{route('depreciation_assets_bulk_remove')}}',
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "order_list":List_ID
                                },
                                ...
                                success: function(data) {
                                    ...
                                    //
                                    // });
                                    $('#damcon_assets_table').DataTable().ajax.reload(null, false);
                                    $('#th_check_box').prop('checked',false);
                                },
                                error: function(data)
                                {
