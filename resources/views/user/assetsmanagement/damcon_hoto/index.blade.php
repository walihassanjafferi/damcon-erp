...
@section('title')
    <title>Damcon ERP -  View Damcon Hoto Management</title>
@endsection
@section('content')
    @include('alert.alert')
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">Damcon Hoto Management</h4>

            <a href="{{ route('damconhoto.create') }}" class="create-btn"><i data-feather='plus'></i> Add Damcon Hoto</a>

            <p class="card-text">
            </p>

            <table class="table table-striped table-bordered damcon_hoto_table" style="width:100%" id="damcon_hoto_table">
                <thead>
                <tr>
                    <th>#</th>
                    <th>ID</th>
...
@section('scripts')
    <script>
        $(document).ready( function () {
            ...
            var table = $('.damcon_hoto_table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('getAjaxhoto') }}",
                columns: [
                    {data: 'checkbox', name: '#'},
                    {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                    {data: 'asset_item_id', name: 'asset_item_id'},
                    {data: 'asset_type', name: 'asset_type'},
                    {data: 'hoto_supervisor', name: 'hoto_supervisor'},
                    {data: 'hoto_date', name: 'hoto_date'},
                    {data: 'handingover_emp_name', name: 'handingover_emp_name'},
                    {data: 'takingover_emp_name', name: 'takingover_emp_name'},
            ...
            $('#damcon_hoto_table tbody').on( 'click', 'tr', function () {
        ...
        function deleteOrder(id){
            ...
            swal({
                title: "Are you sure?",
                text: "Once deleted, you will not be able to recover this Damcon Asset",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                    ...
                    if (willDelete) {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $('#damcon_hoto_table').DataTable().row('.deleted').remove().draw( false );
                            },
                            error: function(data)
                            {
                                alert('Error Failed');
        ...
        function DeleteOrders(){
            ...
            if(List_ID.length>0){
                swal({
                    title: "Are you sure?",
                    text: "Once deleted, you will not be able to recover this Damcon Assets",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                })
                    .then((willDelete) => {
                        if (willDelete) {
                            $.ajax({
                                url:'{{route('damcon_assets_bulk_remove')}}',
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "order_list":List_ID
                                },
